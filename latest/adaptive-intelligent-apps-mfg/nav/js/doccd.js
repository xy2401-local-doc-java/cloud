...
var linkcategories = {
    rank_advanced      : "http://www.oracle.com/pls/cloud/ranked?advanced=1",
    homepage           : "../index.htm",
    master_index       : "http://www.oracle.com/pls/cloud/master_index",
    master_glossary    : "http://www.oracle.com/pls/cloud/master_glossary",
    docindex           : "http://www.oracle.com/pls/cloud/docindex"
  };
...
function leftNav() {
  ...
  function addThisPage() {
    var a, ul, d, p, lastItem, listItem, newList, thisPage, gloss_a,
      ga, g_li, gList, h1, iterate = 0, j = 0, autoId = 0, listStack = [],
      headings = [], glossterms = [], heading, label, labelHide, labelShow;
    ...
    ////heading.style.fontWeight = "bold";
    heading.style.borderBottom = "1px solid #EEE";
    heading.appendChild(document.createTextNode(l10n.this_page));

    heading.style.cursor = "pointer";
    heading.onmouseover = function () {
      document.getElementById("hThisPage").style.display = "inline";
    };
    heading.onmouseout = function () {
      document.getElementById("hThisPage").style.display = "none";
    };
    heading.onclick = function () {
      if (document.getElementById("tThisPage").style.display === "none") {
        document.getElementById("tThisPage").style.display = "block";
        document.getElementById("hThisPage").replaceChild(labelHide, labelShow);
      } else {
        document.getElementById("tThisPage").style.display = "none";
        document.getElementById("hThisPage").replaceChild(labelShow, labelHide);
      }
    };
    ...
    for (iterate = 0; iterate < headings.length; iterate += 1) {
      lastItem = listItem;

      a = document.createElement("a");
      a.href = "#" + headings[iterate].id;
      listItem = document.createElement("li");
      copyContent(headings[iterate], a);
      ...
      if (!lastItem) {
        ...
        ul.style.textIndent = "-0.25em";
        listStack.push(ul);
      } else if (headings[iterate].nodeName > headings[iterate - 1].nodeName) {
        d = headingDifference(headings[iterate], headings[iterate - 1]);
    ...
    if (!listStack[0]) {
      ...
      for (iterate = 0; iterate < p.length; iterate += 1) {
        if (p[iterate].className === "glossterm") {
          gloss_a = p[iterate].getElementsByTagName("a");
          for (j = 0; j < gloss_a.length; j += 1) {
            if (gloss_a[j].id) {
              ga = document.createElement("a");
              ga.href = "#" + gloss_a[j].id;
              ga.appendChild(document.createTextNode(getTextContent(p[iterate])));
      ...
      if (glossterms.length > 0) {
        ...
        for (iterate = 0; iterate < glossterms.length; iterate += 1) {
          gList.appendChild(glossterms[iterate]);
        }
